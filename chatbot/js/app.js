...
function renderQuestion(qData) {
    ...
    // Sostituisci il testo delle domande specifiche
    let questionText = q.question;
    if (q.type === 'text' && (q.question_num === 3 || q.title === 'age')) {
        if (currentLang === 'it') {
            questionText = 'In che anno di scuola 🎒 o università 🎓 ti trovi?';
        } else {
            questionText = 'What year of school 🎒 or university 🎓 are you in?';
        }
    } else if (q.title === 'location') {
        if (currentLang === 'it') {
            questionText = 'Perfetto! In quale località 📍 vorresti fare ripetizioni?';
        } else {
            questionText = 'Perfect! In which location 📍 would you like to have tutoring?';
        }
    }
